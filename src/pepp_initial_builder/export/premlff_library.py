            ...
            rows.append(
                {
                    ...
                    "library_note": "PoreMS native bare_porems_sio2 output; no silanol or hydroxylation post-decoration was applied",
                }
            )
    return rows
...
def build_premlff_structure_library(config: Dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "",
        f"Available relaxed entries: {available}/{len(df)}",
        "",
        ...
        "",
        "Wall chemistry policy: PoreMS native bare_porems_sio2 output is used directly; no silanol generation, hydroxylation, or OH post-decoration is applied.",
        "",
    ]
    if not df.empty:
        cols = ["polymer", "library_status", "lammps_time_ps", "hold_temperature_mean_K", "min_polymer_silica_distance_A", "polymer_inside_pore_fraction", "polymer_silica_contact_count_5p0A", "relaxed_extxyz_path"]
